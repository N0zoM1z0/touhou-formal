  ...
  [ { path := "reference/th07/src/th07/EclManager.hpp"
    ...
    [ { path := "reference/th07/src/th07/EclManager.cpp"
      ...
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1587
        endLine := 1637
        claim := "GET_EXIT_ANGLE selects a player-side random angle and reflects it at movement bounds; its right-positive branch subtracts the enemy's old angle rather than the candidate." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1965
        endLine := 1979
