...
def eclOpcodeInitInterpolation : Int := 27
...
def eclOpcodeBeginSpellcard : Int := 90
def eclOpcodeEndSpellcard : Int := 91
...
def eclOpcodeRunExtension : Int := 121
def eclOpcodeSetExtension : Int := 122
  ...
  [ { path := "reference/th07/src/th07/EclManager.hpp"
    ...
    [ { path := "reference/th07/src/th07/EclManager.cpp"
      ...
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1378
        endLine := 1410
        claim := "Laser-spawn opcodes copy a descriptor from mixed raw and operand-mask-resolved fields, set type 0 only for the moving/aimed opcode, call SpawnLaserPattern, and store the returned pointer in enemy->lasers[laserIdx]." },
      { path := "reference/th07/src/th07/BulletManager.cpp"
        startLine := 665
        endLine := 716
        claim := "SpawnLaserPattern may return early during screen clear, otherwise fills a free laser slot, applies player angle only for type 0, initializes timer/state fields, and returns the Laser pointer." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1265
        endLine := 1329
      ...
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1508
        endLine := 1528
        claim := "SET_BOSS resolves slot 0, writes bosses[slot] for every nonnegative slot without an upper-bound check, but the negative branch hides the GUI only when enemy->bossId < 4." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 667
        endLine := 692
        claim := "BeginSpellcard decodes a 48-byte xor-0xaa name, uses packed i16/u16 sprite/id fields, removes all bullets with item spawning, initializes legacy spellcardInfo, resets stage spell time, resets rank bounds, and computes scoreDrainRate from the timer callback threshold." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 767
        endLine := 852
        claim := "EndSpellcard gates its body on spellcardInfo.isActive, despawns bullets with score max 8000, removes enemies, clears boss special effects, plays the spell-end sound, deactivates legacy spellcardInfo, and marks the stage inactive." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1704
        endLine := 1711
        claim := "SET_BOSS_HEALTH resolves four integer operands and writes GUI boss gauge slot, start/stop ratios divided by enemy->maxLife, and gauge color without an opcode-level slot bound check." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1824
        endLine := 1826
        claim := "SET_NUM_BOSS_LIFE_MARKERS writes a resolved count to the GUI and adds 1800 to playTimeAll." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1899
        endLine := 1900
        claim := "SET_IS_SURVIVAL_SPELLCARD writes the raw low byte into the enemy survival-spellcard flag." },
      { path := "reference/th07/src/th07/EclManager.cpp"
        startLine := 1938
        endLine := 1941
        ...
        { fixedPrefixBytes := 12
          ...
          bossLifecycleOps :=
            [ { opcode := eclOpcodeSetBoss
                ...
                intInputs :=
                  [ { role := .spellSprite
                    ...
                    { role := .spellId
                      ...
                      policy := .rawU16
                      halfIndex := 1 } ]
                spellTextPolicy := some (.xorInline 48 170)
                beginBulletClear := some .removeAllWithItems
                beginStageState := some .starting
                beginSetsLegacySpellInfo := true
                beginResetsStageSpellTimer := true
                beginResetsBulletRank := true
                beginSetsScoreDrainRate := true
                beginRunsSpellcardBackgroundVms := true },
              { opcode := eclOpcodeEndSpellcard
                kind := .endSpellcard
                endRequiresActiveSpell := true
                endStageState := some .inactive
                endBulletClear := some (.despawnWithItems 8000)
                endRemovesEnemies := true
                endDeactivatesLegacySpellInfo := true
                endPlaysSound := true },
              { opcode := eclOpcodeSetBossHealth
                kind := .setBossGauge
                intInputs :=
                  [ { role := .gaugeSlot
                      operandIndex := 0
              ...
              { opcode := eclOpcodeSetNumBossLifeMarkers
                kind := .setLifeMarkerCount
                intInputs :=
                  [ { role := .lifeMarkerCount
                      operandIndex := 0
                      policy := .intRValue } ]
                lifeMarkerTimeBonus := 1800 },
              { opcode := eclOpcodeSetIsSurvivalSpellcard
                kind := .setSurvivalSpellcard
                intInputs :=
                  [ { role := .flagValue
                      operandIndex := 0
                      policy := .rawU8
                      byteIndex := 0 } ] },
              { opcode := eclOpcodeSetBossRunInterrupt
                kind := .setBossRunInterrupt
                intInputs :=
                  [ { role := .runInterruptSlot
                      operandIndex := 0
                      policy := .intRValue },
                    { role := .runInterruptSub
                      operandIndex := 1
                      policy := .intRValue } ]
                bossSlotCount := 8 } ]
          hostEffectOps :=
          ...
          interpolationOps :=
            [ { opcode := eclOpcodeInitInterpolation } ]
          bossIntReads :=
