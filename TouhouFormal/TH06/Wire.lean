...
def eclOpcodeSetVarSelfX : Int := 10
def eclOpcodeSetVarSelfY : Int := 11
def eclOpcodeSetVarSelfZ : Int := 12
...
def eclOpcodeSpellcardStart : Int := 93
def eclOpcodeSpellcardEnd : Int := 94
...
def eclOpcodeExtensionCall : Int := 121
def eclOpcodeExtensionRepeat : Int := 122
def eclOpcodeTimeSet : Int := 123
def eclOpcodeDropItemId : Int := 124
def eclOpcodeStandardUnpause : Int := 125
def eclOpcodeBossSetLifeCount : Int := 126
def eclOpcodeDebugWatch : Int := 127
...
def eclEvidence : List TouhouFormal.SourceRef :=
  [ { path := "reference/th06/src/EclManager.hpp"
    ...
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 536
      endLine := 550
      claim := "BOSSSET writes g_EnemyManager.bosses[raw slot] for every nonnegative slot without an upper-bound check; the negative branch clears bosses[enemy->bossId] and hides the GUI boss flag." },
    { path := "reference/th06/src/EclManager.hpp"
      startLine := 228
      endLine := 233
      claim := "Spellcard-start operands pack signed i16 sprite/id values followed by an inline name tail." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 710
      endLine := 725
      claim := "SPELLCARDSTART shows the spellcard, sets legacy spellcardInfo active/capturing/id/score, turns bullets into point items, marks the stage running, resets the spell timer, and resets bullet-rank bounds." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 749
      endLine := 783
      claim := "SPELLCARDEND runs the active-spell body only when spellcardInfo.isActive, despawns bullets with score max 12800, deactivates legacy spellcardInfo, and always marks the stage not running." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 852
      endLine := 854
      claim := "BOSSSETLIFECOUNT writes the raw boss life marker count to the GUI and adds 1800 to the game time counter." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 923
      endLine := 924
      claim := "SPELLCARDFLAGTIMEOUT writes one raw i32 value into the timeout-spell enemy flag." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 849
      endLine := 850
      claim := "STDUNPAUSE writes one to the stage unpause flag." },
    { path := "reference/th06/src/EclManager.cpp"
      startLine := 916
      endLine := 917
      claim := "ENEMYFLAGINVISIBLE assigns the raw i32 operand to a one-bit enemy flag; DEBUGWATCH has no switch body and therefore reaches ordinary advance." } ]
        ...
        { fixedPrefixBytes := 12
          ...
          bossLifecycleOps :=
            [ { opcode := eclOpcodeBossSet
              ...
              { opcode := eclOpcodeSpellcardStart
                kind := .beginSpellcard
                intInputs :=
                  [ { role := .spellSprite
                      operandIndex := 0
                    ...
                    { role := .spellId
                      ...
                      policy := .rawI16
                      halfIndex := 1 } ]
                spellTextPolicy := some .th06InlineTail
                beginBulletClear := some .turnAllIntoPoints
                beginStageState := some .running
                beginSetsLegacySpellInfo := true
                beginResetsStageSpellTimer := true
                beginResetsBulletRank := true },
              { opcode := eclOpcodeSpellcardEnd
                kind := .endSpellcard
                endRequiresActiveSpell := true
                endStageState := some .inactive
                endBulletClear := some (.despawnWithItems 12800)
                endDeactivatesLegacySpellInfo := true },
              { opcode := eclOpcodeBossSetLifeCount
              ...
              { opcode := eclOpcodeSpellcardFlagTimeout
          ...
          interruptOps :=
            [ { opcode := eclOpcodeSetInterrupt
              ...
              { opcode := eclOpcodeSetCallStackDisabled
                ...
                intPolicy := .rawI32 } ]
          extensionOps :=
            [ { opcode := eclOpcodeExtensionCall
                kind := .callNow
                intPolicy := .rawI32
                tableEntryCount := 17 },
              { opcode := eclOpcodeExtensionRepeat
                kind := .installPerFrame
                intPolicy := .rawI32
                tableEntryCount := 17 } ]
          miscOps :=
            [ { opcode := eclOpcodeStandardUnpause
                kind := .stageUnpause },
              { opcode := eclOpcodeDebugWatch
                kind := .noOp },
              { opcode := eclOpcodeEnemyInvisible
                kind := .writeInt .enemyInvisible (.unsignedBits 1)
                intInputs :=
                  [ { operandIndex := 0, policy := .rawI32 } ] } ]
          intUnaryUpdates :=
          ...
          numericSpecialOps :=
            [ { opcode := eclOpcodeSetVarSelfX
                kind := .copyHostFloat .enemyPositionX
                outputPolicy := .sourceSetVar
                outputOperandIndices := [0] },
              { opcode := eclOpcodeSetVarSelfY
                kind := .copyHostFloat .enemyPositionY
                outputPolicy := .sourceSetVar
                outputOperandIndices := [0] },
              { opcode := eclOpcodeSetVarSelfZ
                kind := .copyHostFloat .enemyPositionZ
                outputPolicy := .sourceSetVar
                outputOperandIndices := [0] } ]
          intDivisorHazards :=
            [ { opcode := eclOpcodeMathIntDiv
