#!/usr/bin/env python3
"""Solve raw ECL integer binary-op paths and materialize Lean-checked fixtures."""
...
def materialize(title: str, path: str, values: dict[str, Any]) -> dict[str, str]:
    ...
    if result.get("matchesPath") != "true":
        raise SymexError(f"materialized integer-binary witness did not replay into requested path: {result}")
    return result


def list_paths() -> list[str]:
    text = checked_stdout(["lake", "exe", "symex", "list-int-binary-paths"])
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solve raw ECL integer binary-op paths with Z3 and materialize Lean-checked fixtures."
    )
    parser.add_argument("title", choices=["th06", "th07", "th08"])
    parser.add_argument("path", help="path name from `lake exe symex list-int-binary-paths`, or `all`")
