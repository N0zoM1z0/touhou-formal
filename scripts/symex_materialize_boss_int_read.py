#!/usr/bin/env python3
"""Solve raw ECL boss integer-read paths and materialize Lean-checked fixtures."""
...
def materialize(title: str, path: str, values: dict[str, Any]) -> dict[str, str]:
    ...
    if result.get("matchesPath") != "true":
        raise SymexError(f"materialized boss integer-read witness did not replay into requested path: {result}")
    return result


def list_paths() -> list[str]:
    text = checked_stdout(["lake", "exe", "symex", "list-boss-int-paths"])
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solve raw ECL boss integer-read paths with Z3 and materialize Lean-checked fixtures."
    )
    parser.add_argument("title", choices=["th06", "th07", "th08"])
    parser.add_argument("path", help="path name from `lake exe symex list-boss-int-paths`, or `all`")
