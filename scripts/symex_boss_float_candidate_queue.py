...
MATERIALIZER = REPO_ROOT / "scripts" / "symex_materialize_boss_float_read.py"
...
def run_materializer(title: str, path: str, active_mask: int, override_mask: int) -> Any:
    ...
    if completed.returncode != 0:
        ...
        raise QueueError(f"boss float-read materializer failed for {title}/{path}: {detail}")
    return json.loads(completed.stdout)


def classify_path(path: str, fixture: dict[str, Any]) -> dict[str, Any]:
    if path in {"boss-float-index-before-array", "boss-float-index-at-or-past-array"}:
        return {
            "class": "boss-index-oob-read",
            "priority": "high",
            "priorityRank": 0,
            "reason": "the source indexes g_EnemyManager.bosses[index] before any TH08 float null guard can run",
        }
    if path == "boss-float-null-deref":
        return {
            "class": "boss-null-deref",
            "priority": "high",
            "priorityRank": 0,
            "reason": "TH07's float boss read can resolve an enemy-dependent float selector through a null boss pointer",
        }
    if path == "boss-float-null-guarded-skip":
        return {
            "class": "boss-null-guarded-skip",
            "priority": "low",
            "priorityRank": 3,
            "reason": "TH08's opcode 87 skips the write when the in-bounds boss slot is null; this is a semantic delta and positive guard control",
        }
    if path == "boss-float-value-raw-no-boss-read":
        return {
            "class": "operand-flag-bypass",
            "priority": "medium",
            "priorityRank": 2,
            "reason": "a clear value operand flag bypasses the boss table read completely and writes the raw slot-1 float bits",
        }
    if fixture.get("valueKind") == "resolved-default-raw":
        return {
            ...
            "priority": "medium",
            "priorityRank": 2,
            "reason": "the opcode reads the boss table but an unknown float selector falls back to the raw operand bits",
        }
    return {
        "class": "boss-read-host-value",
        "priority": "low",
        "priorityRank": 3,
        "reason": "ordinary in-bounds boss read with a resolved host float value",
    }
...
def next_action_for(risk_class: str, title: str, path: str) -> str:
    ...
    if risk_class == "boss-read-default-raw":
        return "compose with metamorphic checks over known versus unknown float selector bit-patterns"
    if risk_class == "boss-null-guarded-skip":
        return "use as a TH08 positive guard control and compare against TH07 null-deref"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a profile-driven symbolic raw ECL boss float-read candidate queue."
    )
    parser.add_argument(
        "--env",
        action="append",
    ...
    parser.add_argument(
        "--path",
        default="all",
        help="path name from `lake exe symex list-boss-float-paths`, or all",
    )
...
def main(argv: list[str]) -> int:
    ...
    for title, active_mask, override_mask, env_name in envs:
        ...
        if not isinstance(records, list):
            raise QueueError("boss float-read materializer returned neither object nor list")
        for record in records:
            if not isinstance(record, dict):
                raise QueueError(f"bad boss float-read materializer record: {record!r}")
    ...
    payload = {
        "schema": "touhou-formal-symex-boss-float-candidate-queue-v1",
        "generator": "scripts/symex_boss_float_candidate_queue.py",
