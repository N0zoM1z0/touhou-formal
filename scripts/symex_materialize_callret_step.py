#!/usr/bin/env python3
"""Solve CALL/RET stack paths and replay them in Lean."""
...
def materialize(title: str, path: str, values: dict[str, Any]) -> dict[str, str]:
    ...
    if result.get("matchesPath") != "true":
        raise SymexError(f"materialized CALL/RET witness did not replay into requested path: {result}")
    return result
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solve CALL/RET stack paths with Z3 and replay in Lean."
    )
    parser.add_argument("title", choices=["th06", "th07", "th08"])
    parser.add_argument("path", help="path name from `lake exe symex list-callret-paths`, or `all`")
