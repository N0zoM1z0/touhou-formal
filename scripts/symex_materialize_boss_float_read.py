#!/usr/bin/env python3
"""Solve raw ECL boss float-read paths and materialize Lean-checked fixtures."""
...
def solve_path(title: str, path: str, active_mask: int, override_mask: int) -> Any:
    ...
    first_line = next((line.strip() for line in z3.stdout.splitlines() if line.strip()), "")
    if z3.returncode != 0 and first_line not in {"unsat", "unknown"}:
...
def materialize(title: str, path: str, values: dict[str, Any]) -> dict[str, str]:
    ...
    if result.get("matchesPath") != "true":
        raise SymexError(f"materialized boss float-read witness did not replay into requested path: {result}")
    return result


def list_paths() -> list[str]:
    text = checked_stdout(["lake", "exe", "symex", "list-boss-float-paths"])
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solve raw ECL boss float-read paths with Z3 and materialize Lean-checked fixtures."
    )
    parser.add_argument("title", choices=["th06", "th07", "th08"])
    parser.add_argument("path", help="path name from `lake exe symex list-boss-float-paths`, or `all`")
