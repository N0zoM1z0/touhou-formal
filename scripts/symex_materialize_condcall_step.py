#!/usr/bin/env python3
"""Solve TH06-style conditional CALL paths and replay them in Lean."""
...
def materialize(title: str, path: str, values: dict[str, Any]) -> dict[str, str]:
    ...
    if result.get("matchesPath") != "true":
        raise SymexError(
            f"materialized conditional CALL witness did not replay into requested path: {result}"
        )
    return result
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solve conditional CALL paths with Z3 and replay in Lean."
    )
