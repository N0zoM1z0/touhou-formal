...
MATERIALIZER = REPO_ROOT / "scripts" / "symex_materialize_raw_step.py"
...
def run_materializer(title: str, path: str, active_mask: int, override_mask: int) -> Any:
    ...
    if completed.returncode != 0:
        ...
        raise QueueError(f"materializer failed for {title}/{path}: {detail}")
...
def classify_path(path: str) -> dict[str, Any]:
    if path == "yielded":
        return {
            "class": "time-gate-control",
            "priority": "control",
            "priorityRank": 4,
            "reason": "current time does not equal instruction time, so dispatch yields without touching opcode semantics",
        }
    if path == "vm-error":
        return {
            "class": "explicit-vm-error",
            "priority": "medium",
            "priorityRank": 2,
            "reason": "source-backed unimplemented opcode reaches the VM error return path",
        }
    if "before-buffer" in path:
        return {
            ...
            "priority": "high",
            "priorityRank": 0,
            "reason": "the next instruction cursor is negative",
        }
    if "at-or-past-end" in path:
        return {
            ...
            "priority": "high",
            "priorityRank": 0,
            "reason": "the next instruction cursor is at or beyond the available raw ECL buffer",
        }
    if "non-progress" in path:
        return {
            ...
            "priority": "high",
            "priorityRank": 1,
            "reason": "the step leaves the instruction cursor at the same file offset",
        }
    return {
        "class": "reachable-control-path",
        "priority": "low",
        "priorityRank": 3,
        "reason": "the path is reachable but remains in-bounds in the current one-step model",
    }
...
def compact_record(record: dict[str, Any], env_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "nextAction": next_action_for(risk["class"], record["title"], env_name, path),
    }


def next_action_for(risk_class: str, title: str, env_name: str, path: str) -> str:
    if title == "th06" and env_name == "retail-lunatic-bit3" and path == "jumped-before-buffer":
        return "retained Wine confirmation exists; use as regression oracle"
    ...
    if risk_class == "explicit-vm-error":
        return "validate whether the retail caller treats ZUN_ERROR as recoverable, dialog, or crash"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a profile-driven symbolic raw ECL candidate queue."
    )
    parser.add_argument(
        "--env",
        action="append",
    ...
    parser.add_argument(
        "--path",
        default="all",
        help="path name from `lake exe symex list-paths`, or all",
    )
...
def main(argv: list[str]) -> int:
    ...
    for title, active_mask, override_mask, env_name in envs:
        ...
        if not isinstance(records, list):
            raise QueueError("materializer returned neither object nor list")
        for record in records:
            if not isinstance(record, dict):
                raise QueueError(f"bad materializer record: {record!r}")
    ...
    payload = {
        "schema": "touhou-formal-symex-candidate-queue-v1",
        "generator": "scripts/symex_candidate_queue.py",
