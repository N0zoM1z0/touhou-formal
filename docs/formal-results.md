...
| `ECL-RAW-STEP-EFFECTIVENESS` | coverage assessment | reruns the raw-step, body, integer resolver, integer-binary, boss-int, CALL/RET, and conditional-CALL candidate queues, checks modeled path coverage per environment, counts local source opcode surface still outside the model, and compares the current formal lane against DanmakuFuzz | `docs/effectiveness.md`; raw 70/70 `sat`, body 85/85 `sat`, resolver 8/8 `sat`, int-binary 39/39 `sat`, boss-int 18/18 `sat`, CALL/RET 41/41 `sat`, conditional-CALL 16/16 `sat`, all `matchesPath=true`, all modeled paths per default environment |
