...
| `ECL-RAW-BODY-RANDOM` | executable model extension | shared random semantics model integer range, range-plus-addend, and parity-sign operations with explicit 32-bit words, while float RNG arithmetic remains a supplied result-bit boundary and TH06 retains `SetVar` RHS re-resolution | Lean theorems check 4 TH06 opcodes, 6 TH07 opcodes, 2 TH08 low-run opcodes, zero-range modulo, TH06 source-write policy, TH07 float result recording, and TH08 negative-sign word wrap |
...
| `ECL-RAW-BODY-ENEMY-STATE` | executable model extension | shared enemy-state effects model primary/secondary hitboxes and collision/damage/death flags while preserving raw/resolved inputs, source bit widths, TH08 mask polarity, alignment-effect mirroring, and presentation-write suppression | Lean theorems check 5 TH06, 7 TH07, and 6 TH08 opcodes; representative controls cover raw versus resolved hitboxes, three-bit truncation, inverted flag masks, alignment mirroring, and suppressed high-opcode death-mode writes |
