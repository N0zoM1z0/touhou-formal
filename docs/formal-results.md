...
| `ECL-RAW-BODY-BULLET-CONTROL` | executable model extension | shared bullet-control effects model all-bullet clearing variants, bullet sound flag/override writes, radius removal, transition clearing, and bullet-rank influence fields | Lean theorems check 3 TH06, 5 TH07, and 3 TH08 opcodes; controls preserve TH06 raw sound/rank operands, TH07/TH08 operand-flag resolution, repeated primary sound reads before override reads, title-specific sound targets, radius resolution, transition clearing, and signed-i16 rank-count truncation |
...
bodies, full host-runtime state, and multi-context scheduling remain outside
the formal semantics. See [`docs/effectiveness.md`](effectiveness.md).
