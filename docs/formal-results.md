...
| `ECL-RAW-BODY-EXTENSION-DISPATCH` | executable model extension | shared semantics models immediate and per-frame EX instruction dispatch over the source's fixed 17/24/32-entry callback tables | Lean theorems cover 2 opcodes per title; controls prove TH06 index 17 faults, TH07 repeats the resolver and can fault on the second index before either install write, TH08 negative indices clear with one read, and index 31 invokes successfully |
