...
`lake exe th06_smt th07-negative-oob | z3 -in` confirms that TH07's unchecked
negative sub id is a counterexample.

`lake exe th06_smt th08-negative-noop-unsat | z3 -in` confirms that the same
negative id is not a counterexample under TH08's source-backed no-op policy.

`lake exe th06_smt th08-positive-oob | z3 -in` keeps TH08's positive upper-bound
fault path live.
